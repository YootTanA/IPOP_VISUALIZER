...
async function requestIPOPData() {
    ...
    $.getJSON(url)
        .then(function (overlays, status) {
            ...
            var overlayID = Object.keys(overlays['current_state'])[0];
            ...
            $.getJSON(allowOrigin + "http://" + serverIP + "/IPOP/overlays/" + overlayID + "/nodes?interval=" + intervalNo + "&current_state=True").then(function (nodes) {
                $.getJSON(allowOrigin +  "http://" + serverIP + "/IPOP/overlays/" + overlayID + "/links?interval=" + intervalNo + "&current_state=True").then(function (links) {
                    var ipopObj = new BuildIPOPData();
                        ...
                        //console.log( links[overlayID]['current_state']['a000001feb6040628e5fb7e70b04f001']);
                        
                    ipopObj.setData(overlays['current_state'][overlayID], nodes[overlayID]['current_state'], links[overlayID]['current_state']);
                  //  console.log( Object.keys( nodes[overlayID]['current_state']));
                      // console.log(ipopObj.getNumNodes());
            ...
            // /* get node and links */
            // $.when(
            //     $.getJSON(allowOrigin + "http://" + serverIP + "/IPOP/overlays/" + overlayID + "/nodes?interval=" + intervalNo + "&current_state=True"),
            //     $.getJSON(allowOrigin +  "http://" + serverIP + "/IPOP/overlays/" + overlayID + "/links?interval=" + intervalNo + "&current_state=True")
            // ).then(function (nodes, links) {
            //     var ipopObj = new BuildIPOPData();
            //     console.log( nodes[overlayID]["current_state"]);

            //    // ipopObj.setData(overlays['current_state'], nodes['current_state'], links['current_state']);
